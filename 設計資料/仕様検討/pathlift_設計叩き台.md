...
研究者がGPMLと設定パラメータを与えると、ツールがオルソログ変換テーブルおよび外部リソース（WikiPathways・NCBI/Ensembl・発現データ）と連携して変換済みGPMLを出力する。BLASTp（TODO:要検討.「配列類似性検索」のように抽象度を少し上げるべきでは. あるいは手法を確定すべきか.BLASTpとは限らないので.）を必要とするケースのみ手動介入が入る。
